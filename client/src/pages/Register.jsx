  ...
  return <div className= "bg-[url('./assets/chairs.jpg')] bg-cover bg-center " >
    ...
    <form className="flex flex-col items-center  w-screen md:w-2/3 m-auto lg:w-2/4 lg:text-lg"  >
      <div className=" flex justify-center lg:w-[100%] md:w-[100%] sm:w-[100%] ">
        <input className={`${styles.regInputs} w-[41%] `} type="text" {...defaults('Fname', 'Förnamn. . .')} />
        <input className={`${styles.regInputs} w-[41%] `} type="text" {...defaults('Lname', 'Efternamn. . .')} />
      </div>
      
      {/* Phone number */}
      <input className={`${styles.regInputs} w-[85%]  `}  type="phone" {...defaults('phone', 'Telefon. . . ', { minLength: 8, type: 'tel' }, 
      ...
      {/* Password */}
      <input className={`${styles.regInputs} w-[85%] `} {...defaults('password', 'Lösenord. . .', { minLength: 8, type: 'password' }, val => /\d/.test(val) && /[A-Z]/.test(val) && /[a-z]/.test(val),
      ...
      {/* Button Submit */}
      <button className="bg-gold w-40 px-4 rounded-lg py-4 my-5 lg:text-lg " type="submit" >Registrera</button>
